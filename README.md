...
### 💡 APOLLO Private の 4 つの要点

APOLLO Private のデータ取り扱いと運用の本質は、次の 4 つに集約されます。

#### 1. 📁 アップロードしたデータはお手元の PC の `apollo-data` フォルダに保存されます

- アップロードした CSV / Excel、計算された分析結果、AI が生成したレポートは
  すべて **ローカルの `apollo-data` フォルダ**に保存されます
- このフォルダは **手元のハードディスク上**に存在します。クラウドではありません
- バックアップはこのフォルダをまるごと USB メモリや NAS にコピーするだけ
- アンインストールはこのフォルダを削除すれば全データが消えます

#### 2. 🚫 特許データがインターネット経由で外部に出ることはありません

- AI 分析も **手元の PC（または社内 GPU マシン）の中だけで完結**します
- Google や OpenAI に特許文面が送信される経路は**コード上に存在しません**
- インターネット接続を切った状態でも動作します（エアギャップ運用）
- Wireshark 等の通信監視ツールで検証可能です
...
#### 4. 🐳 Docker は「アプリを箱に詰めて配布する仕組み」

- 利用者は**箱を作る必要はありません**。すでに Docker Hub に事前ビルド済みの箱（`jl1nie/apollo-private`）が置かれており、セットアップでは自動的にこれをダウンロードして起動します
- セットアップはコマンドを数回実行するだけで完了します
- 一度起動すれば、あとは Web ブラウザで http://localhost:8501 を開くだけ
- 詳しい手順は[「エンドユーザーマニュアル」](#-apollo-private--エンドユーザーマニュアル)で順を追って説明します
...
- **事前ビルド済みイメージ配布** — Docker Hub (`jl1nie/apollo-private`) から pull するだけ。ローカルビルド不要、永続化はマウントボリューム
...
> ※ APOLLO 本体の Docker イメージ（2.05 GB）は Docker Hub から自動的にダウンロードされます。手元でビルドする必要はありません。
...
> 📦 **これは何をしているの？** — ここで取得しているのは APOLLO Private 本体（Docker イメージ）**ではなく**、
> 起動に必要な**設定ファイルのテンプレート**と**セットアップ用のスクリプト**（`just` コマンドや `docker-compose.yml`）です。
> アプリ本体（Docker イメージ）はこのあとステップ 2-5 を実行したときに、Docker Hub から自動でダウンロードされます。
> **あなたがビルドする必要はありません。**
...
> 🚀 **これは何をしているの？** — 初回起動時は Docker Hub から `jl1nie/apollo-private` イメージ（約 2GB）を自動的にダウンロードします。
> ネット回線によっては 5〜15 分ほどかかりますが、2 回目以降はダウンロード済みのイメージがそのまま使われるので数秒で立ち上がります。
> 画面が止まったように見えても、裏でダウンロードが進んでいますので焦らず待ってください。
